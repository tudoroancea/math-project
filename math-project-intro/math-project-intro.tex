...
% Proving such a stability property for a control law $\mu$ is usually tackled using the following classical definition and theorem from dynamical system theory :

% \begin{definition}
% 	For a dynamical system $x^+=\phi(x)$, a \textbf{\textit{global Lyapunov function}} is a function $V:\R^{n_x}\to\R$ such that there exists positive constant $a,b$ and $c$ verifying $\forall x\in\R^{n_x}$:
% 	\begin{subequations}
% 		\label{eq:bruh}
% 		\begin{align}
% 			a\|x\|^2&\leq V(x)\leq b\|x\|^2\\
% 			V(\phi(x))-V(x)&\leq -c\|x\|^2
% 		\end{align}
% 	\end{subequations}
% \end{definition}
...
% 	\begin{enumerate}
% 		\item $x^*$ is an equilibrium point for the dynamics with 0 control : $f(x^*,0)=x^*$
		
% 		\item The costs are 0 at $x^*$ with 0 control : $l(x^*,0)=0$
		
% 		\item $\exists c\geq 0$ s.t. $\forall x\in\cal{X},\forall u\in\cal{U}~:~l(x,u)\geq c(||x-x^*||^2+||u||^2)$
		
% 		\item There exists a control law $\kappa:\cal{X}\to\cal{U}$ s.t. the terminal set $\cal{X}_f\in\cal{X}$ is forward invariant for the dynamical system $x^+=f(x,\kappa(x))$.
		
% 		\item For the same control law $\kappa$, the terminal cost $F:\cal{X}_f\to\R$ is a \textbf{local} Lyapunov function compatible with the stage cost, i.e. $\exists\epsilon>0$ s.t. $\forall x\in\cal{X}_f\cap B_n(0,\epsilon):~F(f(x,\kappa(x)))\leq F(x)-l(x,\kappa(x))$
...
% 	Therefore :
% 	\begin{align*}
% 		&V_N(x^*_1)\leq J_N(\mathbf{x}',\mathbf{u}')=V_N(x)-l(x,u^*_0)\underbrace{-F(x^*_N)+l(x^*_N,\kappa(x^*_N))+F(f(x^*_N,\kappa(x^*_N)))}_{\leq 0\text{ by 5.}}\\
