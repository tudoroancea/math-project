...
A great deal of effort has been put in the past decaded to design algorithms that make best use of scarce computational power to deliver usable results on embedded systems that necessitate very fast response times.
...
We don't explicitely deal with the assumptions necessary for SQP to converge.
...
RTI relies on previsous remarks to find an accurate solution to the MPC problem and combines them with another important idea trying to solve the \textit{real-time dilemma} which can be summarized in the following way :
...
This system desribing an exothermic chemical reaction has 4 states ($c_A$ the concentration of cyclopentadiene (reactant), $c_B$ the concentration of cyclopentenol (product), $\theta$ the temperature in the reactor and $\theta_K$ the temperature in the cooling jacket of the tank reactor) and 2 controls ($u_1$ the scaled feed inflow rate of cyclopentadiene and $u_2$ the heat removal rate of the external heat exchanger designed to cool down the reactor).
...
As suggested in \cite{diehl-dissertation}, we chose a horizon size of 100 and a sampling time of $200.0s$, which corresponds to a prediction of $2000.0s$ in the future.
...
\begin{itemize}[label=\textbullet]
	...
	If we consider the optimization variable to be $z=(\Delta x_0,\dots,\Delta x_N,\Delta u_0, \dots,\Delta u_{N-1})$, and we recall that we have an initial guess $(x_0^{init},\dots,x_{N}^{init},u_0^{init},\dots,u_{N-1}^{init})$ and a current state $x$, then the matrices and vectors in the OSQP formulation will be:\newline
	\scriptsize
	\begin{align*}
		\begin{split}
...
From the previous tables, we can notice two things :
\begin{itemize}[label=\textbullet]
