...
\begin{abstract}
	...
	The theoretical results are complemented by numerical illustrations based on Real Time Iteration.
\end{abstract}
...
We are also going to consider very specific forms of costs and constraints : the costs will always be quadratic ($l(x,u)=x^TQx+u^TRu$ and $F(x)=x^TPx$ for some positive definite matrices $Q,R$ and $P$) and the (inequality) constraints will always be polytopic ($\cal{X}=\left\{x\in\R^{n_x}~|~C_xx\leq d_x\right\},~\cal{U}=\left\{u\in\R^{n_u}~|~C_uu\leq d_u\right\}$ for matrices $C_x,C_u$ and vectors $d_x,d_u$ of appropriate dimension).
...
\begin{itemize}[label=\textbullet]
	...
	However this optimization problem becomes infinite-dimensional so not easily solvable in practice.
...
\begin{proof}~
	...
	We can use the same reasoning as in the paragraph 2.5.5 of \cite[MPC Theory, Computation and Design]{MPC-book} to show that $\tilde{F}$ is a local Lyapunov function, i.e. there exists a neighborhood of the origin such that $\forall x$ in it :
...
In this section, we experimentally verify the stability properties of the RRLB MPC using a concrete system that is often used to benchmark different MPC schemes or solvers : the Continuously Stirred Tank Reactor (CSTR) system.

\subsubsection{Experimental setup}

The CSTR system describes an exothermic chemical reaction has 4 states ($c_A$ the concentration of cyclopentadiene (reactant), $c_B$ the concentration of cyclopentenol (product), $\theta$ the temperature in the reactor and $\theta_K$ the temperature in the cooling jacket of the tank reactor) and 2 controls ($u_1$ the scaled feed inflow rate of cyclopentadiene and $u_2$ the heat removal rate of the external heat exchanger designed to cool down the reactor).
We don't mention here the ODE describing the model, the constraints as well as the MPC costs for brevity purposes, but they can be found in detail in \cite{diehl-dissertation} and \cite{cstr-ref}.
...
\begin{itemize}[label=\textbullet]
	...
	\item As suggested in \cite{diehl-dissertation}, we chose a horizon size of 100 and a sampling time of $200.0s$, which corresponds to a prediction of $2000.0s$ in the future.
	In all our experiments we set the maximum runtime to $5000.0s$ (simulated seconds, since the sampling time is very long, we skipped the parts where nothing happened) and ran the simulation in closed loop (i.e. applying to the system the control computed by the MPC) until the state converged to the reference state within a $10^{-3}$ tolerance in norm.
	...
	We will see in the results that this does not impact convergence.
	...
	This library solves problem of the general form
	\begin{align*}
...
\noindent We designed the following experiments to evaluate the behavior of a given MPC scheme :

\begin{enumerate}[label=Exp\arabic*]
	\item \underline{Verify the theoretical results} :

	We see if the system is indeed LAS when controlled by RRLB MPC and if the constraints are violated.

	\item \underline{Number of iterations to convergence} :

	We see how quickly a scheme makes the state converge to the reference (within a tolerance of $10^{-3}$).

	\item \underline{Compute closed-loop performance measure and compare with infinite horizon MPC} :

	Given by the sum of all the stage costs evaluated at every time step of the simulation.
	For the RTI scheme, this corresponds to the time steps when a measurement is available (see figure \ref{fig:RTI-timeline}), and for the infinite horizon MPC (approximated by a 5 times bigger horizon).

	\item \underline{Compute the runtime of each step of RTI} : i.e. sensitivities computation, condensation
\end{enumerate}

\noindent These experiments were performed with both regular MPC and RRLB MPC to see the advantages of each formulation.
...
\subsection*{Closed-loop trajectories}\label{sec:closed-loop-traj}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_reg_1.png}
	\caption{Closed-loop trajectories for the regular MPC with reference point n°1}
	\label{fig:closed-loop-traj-reg-1}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_rrlb_1.png}
	\caption{Closed-loop trajectories for the RRLB MPC with reference point n°1}
	\label{fig:closed-loop-traj-rrlb-1}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_reg_2.png}
	\caption{Closed-loop trajectories for the regular MPC with reference point n°2}
	\label{fig:closed-loop-traj-reg-2}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_rrlb_2.png}
	\caption{Closed-loop trajectories for the RRLB MPC with reference point n°2}
	\label{fig:closed-loop-traj-rrlb-2}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_reg_3.png}
	\caption{Closed-loop trajectories for the regular MPC with reference point n°3}
	\label{fig:closed-loop-traj-reg-3}
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=16cm]{images/cstr_rrlb_3.png}
	\caption{Closed-loop trajectories for the RRLB MPC with reference point n°3}
	\label{fig:closed-loop-traj-rrlb-3}
\end{figure}
